...
from Binary_tree import generate_binary_tree,tree_is_same
...
class Generator:
    ...
    def generate_fraction(self, num_range):
        ...
        numerator = randint(1, (denominator * num_range))
        if numerator % denominator == 0:
            fra = int(numerator // denominator)
            real_fraction = str(fra)
            ...
            real_fraction = str(Fraction(real_numerator, denominator))
            # 调用fraction方法生成真分数
            real_fraction = str(integer) + "'" + real_fraction
